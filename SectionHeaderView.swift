...
class SectionHeaderView: UITableViewHeaderFooterView {
    ...
    override func awakeFromNib() {
        ...
        self.disclosureButton.setImage(UIImage(named: "2"), for: UIControlState.selected)
    ...
    func toggleOpenWithUserAction(_ userAction: Bool) {
        ...
        if userAction {
            if self.disclosureButton.isSelected {
                if self.delegate.responds(to: "sectionHeaderView:sectionOpened:") {
                    NSLog("Press Open")
                    self.delegate.sectionHeaderView(self, sectionOpened: self.section)
                }
            }
            else {
                if self.delegate.responds(to: "sectionHeaderView:sectionClosed:") {
                    NSLog("Press Close")
